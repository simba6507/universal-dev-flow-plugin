...
> **Status: early / experimental.** The hooks are tested; the multi-agent orchestration is prompt-driven. A cross-language blind benchmark (**6 languages, 32 real bugs**) shows **near-zero false positives**. Recall is modest **when a reviewer sees only the diff (~34%)** — but the dominant lever is **giving the reviewer the intent/contract**: in a controlled test, adding it (nothing else) flipped prior misses **0/6 → 5/6**. Real udflow supplies that intent via its Review Packet, so the blind figure understates it. See the Evidence note. Treat it as a disciplined scaffold.
...
- **The biggest recall lever is feeding the reviewer the intent — and real udflow already does.** In a controlled test, taking 6 bugs a lone reviewer had *missed* and adding only an intent/contract note (the required behavior, never the defect; nothing else changed) flipped **0/6 → 5/6 caught, 0 false positives** — even subtle idiom cases. So ~34% largely reflects *withheld intent*, not a capability ceiling; udflow's **Review Packet** delivers that intent to every reviewer. A multi-lens **panel** (and Deep Mode) recovers more still; a prose "don't rationalize" discipline *alone* was safe but did **not** move recall.
