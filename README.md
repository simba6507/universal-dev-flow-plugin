...
- **Four always-on hooks, invisible during normal work** — a plan-mode write gate, an all-modes destructive-command guard (asks before `rm -rf` / `git reset --hard` / force-push and the like), failure-memory injection at session start, and a best-effort orchestration check at session end. They run in *every* session while the plugin is **enabled** (not only udflow tasks; installing without enabling does nothing); with no Node they no-op. Details under [Plan gate](#plan-gate) and [Failure memory](#failure-memory).
...
| `"udflow": { "destructiveGuard": false }` in a project's `.claude/settings.json` | Opt this project out of the all-modes destructive-command guard (see [Components](#components)) | Behavior: `rm -rf` / `git reset --hard` / `git push --force` etc. no longer prompt for confirmation in this project | Guard **on** (asks; missing/malformed → on, fail-safe) |
...
| `UDFLOW_ENFORCE_STOP=1` | Upgrade the Stop hook's verdict-not-honored advisory to a hard **block** — only when a real gatekeeper blocking verdict **and** an explicit `udflow:delivery=shipped` are both present (never on prose); the model can still hold honestly with `udflow:delivery=held` | Behavior: a blocking verdict can gate session end | Unset (advisory only) |
...
- `udflow/hooks/` — four Node hooks (same behavior on Windows, macOS, Linux; all fail-open):
  ...
  - `destructive-guard.js` (PreToolUse) — in **all** modes, asks for confirmation (never hard-denies) before a narrow, high-confidence set of unrecoverable Bash commands (`git reset --hard`, `git push --force`, `rm -rf`, `find -delete`, `dd of=`, `mkfs`, `shred`); per-project opt-out via `"udflow": { "destructiveGuard": false }`.
  - `load-failure-memory.js` (SessionStart) — injects a failure-memory digest.
  - `orchestration-check.js` (Stop) — best-effort, advisory by default: warns if a `READY` verdict is claimed without the full core review panel running, **or** if the gatekeeper's last verdict was `FIX REQUIRED`/`NOT READY` but the session ends claiming the work is done (an unhonored verdict), **or** if the verification sentinel `udflow:verify=` reports a required check failed/never-ran while the session is delivering. Advisory by default — a Stop hook can't block delivery unless you opt in with `UDFLOW_ENFORCE_STOP` (then only the highest-confidence sentinel-based verdict-not-honored signal becomes a block).
...
- **What each can do:** `plan-gate` can *deny* a Write/Edit/obvious-Bash-write **only while you are in plan mode** (per-project opt-out via `"udflow": { "planGate": false }`); `destructive-guard` can *ask* you to confirm (never deny) a narrow set of unrecoverable Bash commands (`rm -rf`, `git reset --hard`, force-push, `mkfs`, …) in **any** mode (per-project opt-out via `"udflow": { "destructiveGuard": false }`) — best-effort, so obfuscated forms (interpreter one-liners, a word-internal apostrophe) can slip; `load-failure-memory` *reads* your local `FAILURE_MEMORY.md` and injects a nonce-fenced, role-marker-neutralized digest into your own session; `orchestration-check` only *emits an advisory message* at session end (and, **only if you set `UDFLOW_ENFORCE_STOP`**, can *block* session end on the highest-confidence unhonored-verdict signal).
- **What they never do:** change system or security settings, alter file permissions, delete anything (the `destructive-guard` only *prompts before* your own delete/wipe commands — it never deletes), or transmit your code or transcript anywhere.
...
What stays Claude Code-only is **plan-gate enforcement** and the **deep-mode Workflow**: Copilot has no `plan` permission mode, so the plan gate has nothing to enforce and simply **no-ops** there. The four hooks (`plan-gate`, `destructive-guard`, `load-failure-memory`, `orchestration-check`) resolve the plugin root from the environment at runtime and **fail open** (allow, never error) on any host that runs them differently — so under Copilot they degrade gracefully and never break a session. One verified behavioral difference: Copilot CLI **runs** the hooks but does **not surface their injected output to the model** — the `load-failure-memory` SessionStart `additionalContext` (and the `orchestration-check` Stop advisory) are not delivered, so the **auto failure-memory digest is a no-op under Copilot** (the hook still runs, fail-open); and because the Stop output isn't delivered either, the opt-in `UDFLOW_ENFORCE_STOP` hard block is likewise a no-op there — **Stop enforcement is Claude Code-only**. (`destructive-guard` is a PreToolUse decision, not injected output, so its confirmation prompt is the one piece that does apply wherever the harness honors PreToolUse `ask`.) It degrades to the workflow's normal **manual** failure-memory retrieval during planning (the model reads `ai/FAILURE_MEMORY.md` itself), and the readiness discipline is skill/model-driven rather than hook-dependent.
