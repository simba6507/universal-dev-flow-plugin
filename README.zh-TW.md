...
udflow 以 **Claude Code** 為主要目標。它的工作流依賴 Claude Code 的特性——native plan mode（plan gate）、subagent reviewers、`gatekeeper`、deep-mode Workflow——完整的 review/repair 迴圈在那裡才跑得起來。

它的三個 hook（`plan-gate`、`load-failure-memory`、`orchestration-check`）寫成**跨 harness 安全**：在執行時從環境解析 plugin root，並在任何以不同方式執行它們的 host 上**fail open**（放行、絕不報錯）。因此若 udflow 被其他相容 Claude plugin 的 runtime 載入——例如 **GitHub Copilot CLI**——hook 不再把 session 整個擋死；當 Claude 專屬訊號（例如 plan mode）不存在時，它們就單純 no-op。
