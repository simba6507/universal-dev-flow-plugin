...
- **最大的 recall 槓桿是「意圖」,但意圖必須夠具體。** 把**同樣 32 個 bug** 加一段**契約級**意圖註記(寫該做什麼、不寫 bug),recall 從 **11→27 hit(34% → 84%)**、誤報仍 0——連細微慣用陷阱都翻盤。**但 84% 是樂觀上限**:更嚴格的作者偏誤檢查——讓一個**對 bug 全盲**的 agent 只用函式自身簽章/docs 寫意圖——分數**低很多**(籠統的「它做什麼」沒提高 recall;契約級的「必須是位元組數」才有)。所以 recall 取決於 Review Packet 的意圖**有多具體**,不是「有沒有意圖」;真實使用落在 ~34%~84% 之間。一個**乾淨、更大的自動化重跑**(77 bug · 12 repo · 6 語言 · 盲意圖 · 對真實 fix diff 評分)確認了低端:**~29% hit、~39% 碰到、77 個只 1 個誤報**——盲意圖 recall 確實貼近 diff-only 下限,而精準在規模上不變。(**panel** 與 Deep Mode 也能多救一些;單靠「別合理化」措辭紀律**沒**提高 recall。)這份 77-bug 語料也催生了 v0.9.2 兩個 gatekeeper 修正:把「抓到卻被低估」的缺陷重評回真實嚴重度、以及把「缺邊界測試」視為驗證缺口。
