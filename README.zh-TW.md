...
> **狀態:early / experimental(早期/實驗性)。** hook 有測試;多代理編排是 prompt 驅動。跨兩次盲測(C#/.NET + Node/JS,共 13 個真實 bug):**0 誤報**,但抓得率會隨「panel 規模」與「bug 刁不刁」大幅擺動(見下方 Evidence)。請當成有紀律的鷹架。

<details>
<summary><b>Evidence(實測筆記)</b> — 兩次盲測,方向性結果(還不是保證)</summary>

**回溯盲測抓蟲(2026-06-19)。** 把 udflow 的審查員**盲測**在**兩個外部 repo / 兩種語言**(C#/.NET 與 Node/JS,都與本 plugin 不同棧)**共 13 個真實歷史 bug** 的「修復前」程式上。每位審查員只看到有問題的程式片段——看不到修復、issue、或整個 repo——再由一個中立 judge 拿「真相」評分。

| 13 個真實 bug · 2 repo · 2 語言 · 盲測 | 抓到 | 部分 | 漏掉 | 誤報 |
|---|---|---|---|---|
| 結果 | **7** | 3 | 3 | **0**(約 52 個 finding 中) |

- **穩定訊號——0 誤報**,跨兩個 repo、兩種語言(約 52 個 finding;另有約 46 個「看似合理但未驗證」——是廣度訊號,不是已確認的抓到)。
- **抓得率變動極大。** Run 1(C#,有 reviewer panel、多個程式碼可見缺陷)抓到 **6/8**;Run 2(Node/JS,**單一** reviewer、bug 更刁)只抓到 **1/5**(另 2 個 partial)。高度取決於 (a) 跑**完整 panel** vs 單人、(b) bug 有多刁。
- **抓得好:** 具體、程式碼裡看得到的缺陷——資源洩漏、DB 欄位溢位、對 path-only 輸入會 throw 的 `new URL()`、形同虛設的表單驗證器、灌進搜尋索引的原始 JSON。
- **弱點:** 語言慣用陷阱(錯誤的 `this` 綁定)、領域知識型(RFC / 依賴行為)、遺漏型/意圖型(漏清子表;統計該排除 binary)。完整三人 panel **也救不回**遺漏型——槓桿是把**意圖/spec 餵給審查員**,不是加人。有時審查員**有抓到、卻把嚴重度評太低**(判 partial)。

**侷限:** n=13、兩個 repo;不少 bug 取自 `fix` commits(Run 1 有幾個本來就被 review 抓過;Run 2 全部來自 issue/PR 修復);並行/整合型幾乎沒測;審查員**沒拿到 plan/需求脈絡**、Run 2 還是**單一審查員**——兩者都**低估**真實 udflow 流程。**方向性,非保證。**

**畢業條件**——記錄在 [`EVIDENCE.md`](EVIDENCE.md)(手動 log;udflow 不含任何遙測)。只有當該 log 累積到 **≥3 個外部 repo、≥2 種語言、≥20 個合格資料點**(盲測抓蟲或已驗證的真實任務)、算得出抓蟲率與誤報率,**且其中至少一半是並非由 review 先抓出的 bug**,才拿掉「experimental」。嚴格認定:只有「**有標準答案、可檢驗**」的 run 才算——口碑/採用另記、不算進比率。_目前:2 repo · 2 語言 · 13 點(語言 ✓、反偏誤 ✓;還差 ≥3 repo 與 ≥20 點)。_
