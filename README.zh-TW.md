...
> **狀態:early / experimental(早期/實驗性)。** hook 有測試;多代理編排是 prompt 驅動。跨語言盲測(**6 種語言、32 個真實 bug**)顯示:**近乎零誤報**。**只給 diff 時**抓得率不高(約 34%)——但最大的槓桿是**把意圖/契約餵給審查員**:對照實驗中,只加意圖(其餘不變)就讓原本漏掉的 **0/6 → 5/6**。真實 udflow 透過 Review Packet 本來就會帶意圖,所以這個盲測數字**低估**了它(見下方 Evidence)。請當成有紀律的鷹架。
...
- **拉高 recall 最大的槓桿是「餵意圖」,而真實 udflow 本來就會。** 對照實驗:拿 6 個單人**漏掉**的 bug,只加一段「意圖/契約」註記(寫該做什麼、不寫 bug;其餘都不變),就翻成 **0/6 → 5/6 抓到、0 誤報**——連細微慣用陷阱都翻盤。所以約 34% 主要反映**被抽掉的意圖**,不是能力天花板;udflow 的 **Review Packet** 本來就把意圖交給每個審查員。**多視角 panel**(與 Deep Mode)再多救一些;單靠「別合理化」的措辭紀律雖**安全但沒**提高 recall。
