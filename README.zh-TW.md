...
**畢業條件**——記錄在 [`EVIDENCE.md`](EVIDENCE.md)(手動 log;udflow 不含任何遙測)。只有當該 log 累積到 **≥3 個外部 repo、≥2 種語言、≥20 個合格資料點**(盲測抓蟲或已驗證的真實任務)、算得出抓蟲率與誤報率,**且其中至少一半是並非由 review 先抓出的 bug**,才拿掉「experimental」。嚴格認定:只有「**有標準答案、可檢驗**」的 run 才算——口碑/採用另記、不算進比率。_目前:1 repo · 1 語言 · 8 點。_
