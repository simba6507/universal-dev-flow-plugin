...
udflow 以 **Claude Code** 為目標;其 **subagents** 與 **skills** 也能在 **GitHub Copilot CLI** 載入（1.0.65 實機驗證,2026-06-26 —— `plugin list`、`skill list`、列舉全部 subagent、當時隨附的四個 hook 觀察到觸發；之後新增的 `precompact-fidelity.js`（PreCompact）已單元測試,但尚未在 Copilot 下實機冒煙）。跨 harness 載入依各工具有文件記載的 plugin 格式推得。
