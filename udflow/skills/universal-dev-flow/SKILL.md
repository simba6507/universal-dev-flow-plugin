...
8. Auto-fix loop
   ...
   - **Long-run progress ledger (optional).** On a long multi-iteration run, keep a short ledger at `output/udflow/progress.md` — one line per completed step keyed by the commit (short SHA) it produced (e.g. `<sha> · <step> · verify=<pass|fail> · <verdict-so-far>`). After a context compaction, re-read it (and `git log`) before redoing work, so finished, committed steps are not repeated. It is a kept run artifact under `output/udflow/`, gitignored, never committed into a distributed repo (`references/verification-gate.md`, Artifact Hygiene). The ledger is the persistence layer; the `precompact-fidelity.js` PreCompact hook additionally injects a reminder to carry udflow's in-context constructs (verdicts, acceptance-criteria state, `[unverified]` flags, Run Card numbers, subagent findings, unanswered requirements) through the summary, and the existing SessionStart/`compact` path is the restore trigger. On a harness that doesn't surface SessionStart/PreCompact context (e.g. Copilot CLI), re-read the ledger manually after a compaction (the same manual fallback failure memory uses there).
