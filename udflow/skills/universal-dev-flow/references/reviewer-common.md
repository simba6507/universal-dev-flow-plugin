...
- **Tag a genuine hunch `[unverified]`; never inflate it.** If you suspect a defect but your read-only sandbox genuinely cannot run the input that would confirm it, you may still surface it as a *lead to probe* — prefixed with the literal `[unverified]` and stating what evidence would confirm it — but it must NOT be filed as `blocker` (cap it at `minor`, or `major` only with a clearly named mechanism). Prefer converting it to a concrete case (run the input) over shipping the hunch: `[unverified]` is the honest channel for what you genuinely cannot test, not a lane to dodge a reproducible bug. The `gatekeeper` downranks `[unverified]` findings, so an un-evidenced hunch never gates release — which is exactly why an un-tagged finding you *could* have reproduced is the weaker move.
...
- **Look for omissions, not only wrong lines.** Compare what the code does against what its name, signature, docstring, and the requirement imply it must do, and flag what is missing — an unhandled case, a cleanup that should pair with a create/delete, a guard/limit/validation the contract requires, a value (length, default, header) that should be set. Anchor every omission finding to that implied contract; do not speculate beyond it. **Read both sides of a crossed contract:** when the change touches one end of a boundary (an API response shape vs the consumer's type, a route/path vs its href, a state-map key vs its update, a producer vs its consumer, a caller vs a callee signature), open the *other* end in the same pass and confirm they still agree — a boundary mismatch is a top omission source and is invisible from one side. **Grep-verify before asserting an omission:** before filing "X is missing", `Grep`/`Read` the changed tree to confirm X is genuinely absent — not merely absent from the filtered diff (which is a starting point, not the whole tree). An omission a quick search would refute is a false positive, and the precision axiom (a false positive is worse than a documented miss) applies to omission findings too. **deferred ≠ missing:** work the packet's *Out of scope* lists, or that the plan/user explicitly deferred, is **not** a missing-omission — do not flag intentionally-deferred, out-of-scope, or pending follow-up work as an omission. Flag only what the *in-scope* contract implies and is genuinely absent.
